...
public class Board extends JPanel implements ActionListener, KeyListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (upPressed) player.move(0, -1);
        if (downPressed) player.move(0, 1);
        if (leftPressed) player.move(-1, 0);
        if (rightPressed) player.move(1, 0);
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (key == KeyEvent.VK_UP) {
            upPressed = true;
            player.setSprite(player.getSprite("front"));
            playerView.loadImage();
            player.move(0, -1);
        }
        if (key == KeyEvent.VK_RIGHT) {
            rightPressed = true;
            player.setSprite(player.getSprite("right"));
            playerView.loadImage();
            player.move(1, 0);
        }
        if (key == KeyEvent.VK_DOWN) {
            downPressed = true;
            player.setSprite(player.getSprite("back"));
            playerView.loadImage();
            player.move(0, 1);
        }
        if (key == KeyEvent.VK_LEFT) {
            leftPressed = true;
            player.setSprite(player.getSprite("left"));
            playerView.loadImage();
            player.move(-1, 0);
        }
    }
